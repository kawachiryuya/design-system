...
- **VR 集約モデル移行 (composites バッチ 6: layout): AppShell / TwoColumn / SplitPane** (story id 変更 = silent break §10-2): 旧 `Variants` を `Overview` にリネーム (contentMax/split/listWidth の prop カタログ)。`EdgeCases` は slot 省略 / full-bleed / sticky / empty 等、slot・className 駆動で Playground では作れない構造ケースとして維持。meta の `chromatic.viewports=[375,1280]` を維持し mobile/desktop の breakpoint 挙動 (サイドバー出し分け・縦スタック↔grid) を撮る。`?path=/story/composites-appshell--variants`・`composites-twocolumn--variants`・`composites-splitpane--variants` の旧 Storybook URL が `--overview` に変わる。
