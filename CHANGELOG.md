...
- **VR 集約モデル移行 (composites バッチ 2 前半): Avatar / Breadcrumb** (story id 削除 = silent break §10-2): 旧カタログ (`Variants` / `Sizes` / `States` 等) を `Overview` に統合。Avatar Overview = content fallback (image→initials→placeholder) / size / shape / status の内在軸 (user card / avatar group 等 usage 合成は guideline へ)。Breadcrumb は唯一の視覚軸 separator (3 種) のみを Overview に凍結し、階層数・長文 truncate・link hover/focus は Controls で再現でき固有シグナルも足さないため VR 対象外 (EdgeCases は持たない)。`?path=/story/composites-avatar--variants`・`composites-breadcrumb--*` 等の旧 Storybook URL が無効になる。
- **EdgeCases を尖らせた基準で再点検: Button / Input / Link** (story id 削除 = silent break §10-2): 早期移行した primitives の EdgeCases を §5-3 の尖らせた基準 (Controls で再現できる長文・fullWidth は除外、構造ケースのみ) で見直し。Button は fullWidth / 長文折返しを削除し inner-span truncate のみ残す。Input は fullWidth+長文 placeholder が Controls 再現可のため `EdgeCases` story を削除 (Playground + Overview の 2 節へ)。Link は長文折返し / external+長文を削除し段落中インラインのみ残す。`?path=/story/primitives-input--edge-cases` の Storybook URL が無効になる。
- **VR 集約モデル移行 (composites バッチ 7: overlay): Modal / Popover / DropdownMenu / Tooltip / Toast** (story id 削除 = silent break §10-2): overlay 特例 (§7-10) で開状態を静的に凍結。Modal / Popover / DropdownMenu は top-layer が 1 つだけのため代表 1 枚を `open` 固定で `Overview` 化 (Modal=標準ダイアログ / Popover=rich content / DropdownMenu=icon+disabled+destructive を 1 パネルに集約)、各 Playground は `disableSnapshot` に切替。Modal の Sizes/States/EdgeCases、Popover の Placements/RichContent/Controlled、DropdownMenu の WithIcons/States/Controlled story を削除し size/placement/usage は Playground + guideline で担保。Tooltip (`open` prop 無し) と Toast (`position:fixed` + 入場アニメ) は安定 VR ができないため開状態 Overview を見送り Playground を撮影対象に残置 (正攻法の最小 API 追加は #90)。Modal の InitialFocus story は削除 (initialFocusRef は guideline アクセシビリティ節で説明済み)。Modal / Tooltip の Playground を args 連動にし Controls を operable に (§5-3、DropdownMenu/Popover/Toast の Controls + 視覚調整は #91)。`?path=/story/composites-modal--sizes`・`composites-modal--initial-focus`・`composites-popover--placements`・`composites-dropdownmenu--with-icons` 等の旧 Storybook URL が無効になる。
- **VR 集約モデル移行 (composites バッチ 6: layout): AppShell / TwoColumn / SplitPane** (story id 変更 = silent break §10-2): 旧 `Variants` を `Overview` にリネーム (contentMax/split/listWidth の prop カタログ)。`EdgeCases` は slot 省略 / full-bleed / sticky / empty 等、slot・className 駆動で Playground では作れない構造ケースとして維持。viewport は **EdgeCases のみ `[375,1280]`、Overview は `[1280]`** (prop カタログ contentMax/split/listWidth は desktop 機能で mobile では縦積みに潰れ重複するため)。mobile↔desktop の構造切替 (サイドバー出し分け / mobileReverse / mobile で list 非表示) は EdgeCases で検証。AppShell Overview から showBottomNav 行 (mobile 専用) を除去。`?path=/story/composites-appshell--variants`・`composites-twocolumn--variants`・`composites-splitpane--variants` の旧 Storybook URL が `--overview` に変わる。
- **VR 集約モデル移行 (composites バッチ 5: ナビ・開閉): Pagination / Tabs / Accordion** (story id 削除 = silent break §10-2): 旧 `States` / `Variants` / `EdgeCases` を `Overview` に統合。Pagination = 構造バリエーション (first/last/few/many=ellipsis/showEdges) / Tabs = tab 状態 (active/badge/disabled) + panel、多数タブの固定幅 overflow のみ `EdgeCases`、`ManualActivation` は挙動テストで見た目が Overview と同一のため `disableSnapshot` / Accordion = type (single/multiple) + states (collapsed/expanded/disabled/hover/focus、開状態を defaultOpenIds で静的化)。記事一覧 / プロフィール / FAQ ページ等の usage 合成は各 guideline の「使用例」へ移設。`?path=/story/composites-pagination--states`・`composites-tabs--states`・`composites-accordion--variants` 等の旧 Storybook URL が無効になる。
- **VR 集約モデル移行 (composites バッチ 4: 入力・選択): FilterChip / NumberInput / SearchBar / SegmentedControl / Select** (story id 削除 = silent break §10-2): 旧 `States` / `Sizes` / `WithIcon` / `EdgeCases` を `Overview` に統合。FilterChip = 状態×interaction マトリクス + icon 構成 / NumberInput = size + states (at-min/at-max/3桁) / SearchBar = size + states (empty/値あり/disabled/focus、loading=spinner はアニメで VR が揺れるため除外) / SegmentedControl = 状態×interaction + icon、多数セグメントの固定幅オーバーフローのみ `EdgeCases` / Select = size + states (required の * 含む)。Select の optgroup・長 options は native ドロップダウンを開いた時のみ見え静的に撮れないため VR 非対象 (guideline へ)。フィルターバー / 予約フォーム / ヘッダー検索 / ダッシュボード期間切替 / 住所フォーム等の usage 合成は各 guideline の「使用例」へ移設 (Layout token デモは Tokens/Layout 参照)。`?path=/story/composites-filterchip--states`・`composites-numberinput--sizes`・`composites-searchbar--states`・`composites-segmentedcontrol--with-icon`・`composites-select--sizes` 等の旧 Storybook URL が無効になる。
- **VR 集約モデル移行 (composites バッチ 3: 選択コントロール): Checkbox / Radio / Switch / ToggleButton** (story id 削除 = silent break §10-2): 旧 `States` / `EdgeCases` を `Overview` に統合。各 Overview は状態 × interaction (focus-visible / hover を pseudo 強制) のマトリクス + 付帯要素。Checkbox のみ CheckboxGroup (fieldset/legend + error の Context 伝播 = 複数インスタンス構造) を `EdgeCases` に残す。会員登録 / 注文フォーム / 設定パネル / 座席グリッド / 曜日セレクター等の usage 合成は各 guideline の「使用例」へ移設 (Layout token デモは Tokens/Layout 参照)。`?path=/story/composites-checkbox--states`・`composites-radio--states`・`composites-switch--states`・`composites-togglebutton--states`・各 `--edge-cases` 等の旧 Storybook URL が無効になる。
- **VR 集約モデル移行 (composites バッチ 2 後半): EmptyState / ProgressBar** (story id 削除 = silent break §10-2): 旧 `Sizes` / `States` / `Variants` / `EdgeCases` を `Overview` に統合し、両者とも構造的 EdgeCase を持たない 2 節 (Playground / Overview) に。EmptyState Overview = size (比例スケール) / composition (title/description/action/secondaryAction の有無)、custom icon は Playground の `icon` mapping で探索。ProgressBar Overview = color (value 値域も兼ねる) / size / label 有無 (value 端は color 行が兼ね、indeterminate はアニメで VR が揺れるため除外)。検索結果なし・エラー・空フォルダ・アップロードアニメ・ステップバー・wizard 等 usage 合成は各 guideline の「使用例」へ移設 (Layout token デモは Tokens/Layout 参照)。`?path=/story/composites-emptystate--sizes`・`composites-progressbar--variants` 等の旧 Storybook URL が無効になる。
...
- **mode①(no-Tailwind) の最小実証サンプル + CI ガード** (#95): `examples/no-tailwind/` に Vite + React (Tailwind ゼロ) の最小アプリを追加。公開エントリ + `styles.css` / `variables.css` の 2 ファイルだけで AppShell (レスポンシブなサイドバー) + Button / Card / フォーム / Modal を描画し、mode① を end-to-end 検証する。CI (`verify` ジョブ) に「DS build → サンプル build」を追加し、exports / 配信 CSS の silent break (§10-2) を今後機械検出する。`Introduction` の「モード 1」節から導線。サンプルはパッケージ (`files`) に同梱しない。
- **`data-ds-root` マーカーを全コンポーネント / overlay root に付与** (新 DOM 契約 = §10-6(c)、MINOR、#76): 配信ベースリセットを DS サブツリー内だけに scope するための識別属性。全 44 コンポーネントの最外要素と overlay (Modal=`<dialog>` / Popover / Tooltip / DropdownMenu のパネル) に付く。以後この属性は silent-break 面 (§10-2) として扱う。

### Fixed

- **配信 CSS (mode①): Tailwind utility / フォームが動く最小 base を補完し、DS サブツリーに scope** (silent break §10-2 / 配信契約 §10-6(c)、#95 / #76): `styles.css` は preflight OFF (#57) のため、`border-style:solid` / `box-sizing:border-box` / フォーム正規化が欠落し、mode① 消費で **border が消える (button=outset / input=inset の UA 値、checkbox の枠が不可視)・`<input>` が content-box でズレる**不具合があった (examples/no-tailwind で発覚)。opinionated な preflight (見出し/リスト/body margin 等) は入れず、**utility と native フォームが動く前提だけの最小 base** ([`styles/base.css`](./styles/base.css)) を追加。さらに `*`/素タグの **global ではなく `:where([data-ds-root]…)` に scope** し (特異性 0 を維持して utility が勝つ)、**消費者の素の要素には触れない** (C-scoped、#76)。`check:styles` に「最小 base の scope 存在」「global footprint 非混入」を assert。Storybook (本物の preflight 所持) は影響なし。
